...
请按类别整理输出。`,
  ...
  component: `请���取这张原理图中的所有元器件清单（BOM），格式如下：
【主芯片】
- 型号：
- 封装：
 ...
 * 将PDF页面渲染为Base64图片
 */
async function renderPageToBase64(page, scale = 2.0) {
  ...
  try {
    ...
    // 忽略某些警告性错误
    if (!error.message?.includes('transparent')) {
      throw error
    }
  }
 ...
 * 使用VLM分析原理图
 */
async function analyzeSchematic(base64Image, promptType = 'analysis') {
...
async function testSchematic() {
  ...
  const pageNum = args[1] ? parseInt(args[1]) : 2 // 默认第2页（主原理图页）
  const promptType = args[2] || 'analysis' // 默认使用分析模式

  console.log('='.repeat(60))
  console.log('原理图智能分析测试')
  console.log('='.repeat(60))
  ...
  console.log('\n[2/3] 调用PaddleOCR-VL-1.5进行分析...')
  const startTime = Date.now()
  const analysis = await analyzeSchematic(base64Image, promptType)
...
testSchematic().catch(console.error)
